...
export function App() {
  ...
  return <div className="app-shell">
    ...
    {view === "decisions" ? <DecisionInbox messages={agentMessages} attentionPolicy={attentionPolicy} onRead={async id => { try { const result = await api.markAgentMessageRead(id); setAgentMessages(current => current.map(message => message.id === id ? result.message : message)); } catch (cause) { setError(String(cause)); } }} onDismiss={async id => { try { const result = await api.dismissAgentMessage(id); setAgentMessages(current => current.map(message => message.id === id ? result.message : message)); } catch (cause) { setError(String(cause)); } }} onAction={async (id, actionId) => { try { const result = await api.actOnAgentMessage(id, actionId); setAgentMessages(current => current.map(message => message.id === id ? result.message : message)); void notify({ kind: "system", title: "Agent response recorded", body: actionId }); } catch (cause) { setError(String(cause)); } }}/>
      ...
      : view === "mobile" ? <MobileDecisionTerminal messages={agentMessages} attentionPolicy={attentionPolicy} presence={presence}/>
...
function AndroidDeviceHealth() {
  const [status, setStatus] = useState<NormalizedOperatorStatus | null>(null);
  return <section className="settings-card device-health" aria-label="Android / Fabric Device Health"><div className="settings-card-head"><div className="settings-icon"><Icon name="nodes"/></div><div><h2>Android / Fabric Device Health</h2><p>Read-only emulator status from the ROM lab bridge. This status is advisory and cannot grant authority or trigger actions.</p></div></div>{status ? <DeviceHealthBody normalized={status} onClear={() => setStatus(null)}/> : <div className="device-health-empty"><p className="settings-source">No live bridge transport connected yet.</p><button className="button secondary" onClick={() => setStatus(parseOperatorStatus(SAMPLE_OPERATOR_STATUS))}>Show sample status</button></div>}</section>;
}

function MobileDecisionTerminal({ messages, attentionPolicy, presence }: { messages: AgentMessage[]; attentionPolicy?: AttentionPolicy; presence: PresenceSnapshot }) {
