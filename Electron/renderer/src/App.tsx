...
import type { AgentAction, AgentChannelStatus, AgentMessage, AttentionEvent, AttentionPolicy, BackendConnection, CallRow, ConfigStatus, Contact, ContactPoint, ContactPolicy, ContactTimelineItem, DataStatus, DesktopStatus, McpStatus, Message, PresenceSnapshot, SignalItem, SignalSubscription, Thread, View } from "./types";
...
function Rail({ view, running, agentUnreadCount, onView }: { view: View; running: boolean; agentUnreadCount: number; onView(view: View): void }) {
  ...
  const activeFor = (item: View) => view === item || (item === "channels" && ["messages", "calls", "signals", "mobile"].includes(view));
...
function Channels({ threads, calls, signalItems, signalSubscriptions, config, agentChannels, onView }: { threads: Thread[]; calls: CallRow[]; signalItems: SignalItem[]; signalSubscriptions: SignalSubscription[]; config?: ConfigStatus; agentChannels: AgentChannelStatus[]; onView(view: View): void }) {
  ...
  return <main className="content-panel page-panel settings-page"><header className="page-header"><div><span className="eyebrow">Communication surfaces</span><h1>Channels</h1><p>Messages, voice, feeds, and provider configuration live here; decisions stay in their own queue.</p></div></header><div className="settings-grid"><section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="chat"/></div><div><h2>Messages</h2><p>{threads.length} conversations · {unreadThreads} unread.</p></div></div><button className="button primary full" onClick={() => onView("messages")}>Open messages</button></section><section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="phone"/></div><div><h2>Calls</h2><p>{calls.length} call records.</p></div></div><button className="button secondary full" onClick={() => onView("calls")}>Open calls</button></section><section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="rss"/></div><div><h2>Signals</h2><p>{signalSubscriptions.length} sources · {signalItems.length} latest items.</p></div></div><button className="button secondary full" onClick={() => onView("signals")}>Open signals</button></section><section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="phone"/></div><div><h2>Mobile terminal</h2><p>Paired, redacted, signed human decisions for Tauri 2 mobile.</p></div></div><button className="button secondary full" onClick={() => onView("mobile")}>Open mobile terminal</button></section><section className="settings-card span-two"><h2>Provider configuration</h2><div className="status-list"><StatusRow label="Local native channel" ready={true}/><StatusRow label="SMS/MMS provider" ready={config?.account_sid && config?.auth_token && config?.phone_number}/><StatusRow label="Voice webhook" ready={voiceReady(config)}/><StatusRow label="Agent channels" ready={agentChannels.some(channel => channel.enabled && channel.configured)}/></div></section></div></main>;
}
