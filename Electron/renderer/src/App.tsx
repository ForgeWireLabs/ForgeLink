...
import type { AgentAction, AgentChannelStatus, AgentMessage, AttentionEvent, AttentionPolicy, BackendConnection, ConfigStatus, Contact, DataStatus, DesktopStatus, McpStatus, Message, SignalItem, SignalSubscription, Thread, View } from "./types";
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    {modal?.kind === "contact" && <Modal title="Add contact" eyebrow="Contacts" submitLabel="Save contact" onClose={closeModal} onSubmit={async data => { await api.saveContact(String(data.get("name")), String(data.get("number"))); await loadAll(); void notify({ kind: "system", title: "Contact saved", body: String(data.get("name")) }); }}><div className="form-stack"><Field label="Name"><input name="name" placeholder="Contact name" required autoComplete="name"/></Field><Field label="Phone number"><input name="number" type="tel" defaultValue={modal.number} placeholder="+1 555 123 4567" required autoComplete="tel"/></Field></div></Modal>}
    {modal?.kind === "contact-edit" && <Modal title="Edit contact" eyebrow={modal.contact.number} submitLabel="Save changes" onClose={closeModal} onSubmit={async data => { await api.updateContact(modal.contact.id, { name: String(data.get("name")), company: String(data.get("company")), role: String(data.get("role")), tags: String(data.get("tags")), notes: String(data.get("notes")), trust_level: String(data.get("trust_level")), pinned: data.get("pinned") === "on", favorite: data.get("favorite") === "on" }); await loadAll(); void notify({ kind: "system", title: "Contact updated", body: String(data.get("name")) }); }}><div className="form-stack"><Field label="Name"><input name="name" defaultValue={modal.contact.name} required autoComplete="name"/></Field><Field label="Company"><input name="company" defaultValue={modal.contact.company || ""} placeholder="Organization"/></Field><Field label="Role"><input name="role" defaultValue={modal.contact.role || ""} placeholder="Title or relationship"/></Field><Field label="Tags" hint="Comma-separated labels."><input name="tags" defaultValue={modal.contact.tags || ""} placeholder="vip, family"/></Field><Field label="Notes"><textarea name="notes" rows={3} defaultValue={modal.contact.notes || ""} placeholder="Context worth remembering"/></Field><Field label="Trust level"><select name="trust_level" defaultValue={modal.contact.trust_level || "unknown"}><option value="unknown">Unknown</option><option value="known">Known</option><option value="trusted">Trusted</option><option value="operator">Operator</option><option value="blocked">Blocked</option></select></Field><label className="toggle-row"><input type="checkbox" name="pinned" defaultChecked={Boolean(modal.contact.pinned)}/><span>Pinned</span></label><label className="toggle-row"><input type="checkbox" name="favorite" defaultChecked={Boolean(modal.contact.favorite)}/><span>Favorite</span></label><button type="button" className="button danger full" onClick={async () => { if (!window.confirm(`Delete ${displayName(modal.contact)}? This removes the contact and its numbers.`)) return; await api.deleteContact(modal.contact.id); await loadAll(); closeModal(); void notify({ kind: "system", title: "Contact deleted", body: displayName(modal.contact) }); }}>Delete contact</button></div></Modal>}
