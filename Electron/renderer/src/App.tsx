...
import type { AgentAction, AgentMessage, BackendConnection, ConfigStatus, Contact, DataStatus, DesktopStatus, McpStatus, Message, Thread, View } from "./types";
...
export function App() {
  ...
  const loadAll = useCallback(async () => {
    const [nextThreads, nextContacts, nextAgentMessages, nextConfig, nextDataStatus, nextMcpStatus] = await Promise.all([api.threads(), api.contacts(), api.agentMessages(), api.config(), api.dataStatus(), window.desktop?.mcpStatus()]);
    setThreads(nextThreads); setContacts(nextContacts); setAgentMessages(nextAgentMessages); setConfig(nextConfig); setDataStatus(nextDataStatus); if (nextMcpStatus) setMcpStatus(nextMcpStatus);
