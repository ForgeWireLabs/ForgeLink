...
import type { AgentAction, AgentChannelStatus, AgentMessage, AndroidPairingStatus, AttentionEvent, AttentionPolicy, BackendConnection, CallRow, ConfigStatus, Contact, ContactPoint, ContactPolicy, ContactTimelineItem, DataStatus, DesktopStatus, EmailChannelStatus, EmailSettingsInput, EmailSettingsStatus, ForgeLinkNodeLinkStatus, McpStatus, Message, OutboundDraft, PresenceSnapshot, PushChannelStatus, PushSettingsInput, PushSettingsStatus, RedactionPreview, SampleStatus, SignalItem, SignalSubscription, Thread, View } from "./types";
...
const NODE_CAPABILITY_LABELS: Record<ForgeLinkNodeLinkStatus["link_state"], { messages: string; contacts: string; calls: string; signals: string; decisions: string; sync: string; push: string; }> = {
  local_only: {
...
function ExtLink({ href, children }: { href: string; children: ReactNode }) { return <a className="help-link" href={href} onClick={event => { event.preventDefault(); shell.openExternal(href); }}>{children}</a>; }
...
export function App() {
  ...
  const applyMobileRuntimeGap = useCallback((cause: unknown) => {
    ...
    void shell.agentChannels().then(setAgentChannels).catch(() => setAgentChannels([]));
    void shell.pairingStatus().then(setPairingStatus).catch(() => setPairingStatus(DEFAULT_ANDROID_PAIRING_STATUS));
    void shell.nodeLinkStatus().then(setNodeLinkStatus).catch(() => setNodeLinkStatus(DEFAULT_NODE_LINK_STATUS));
    setOutboundDrafts([]);
    setSampleStatus(undefined);
    setEmailStatus(undefined);
    setPushStatus(undefined);
    void shell.attentionPolicy().then(setAttentionPolicy).catch(() => setAttentionPolicy(current => current || DEFAULT_ATTENTION_POLICY));
  ...
  const loadAll = useCallback(async () => {
    const [nextThreads, nextContacts, nextAgentMessages, nextCalls, nextSignalSubscriptions, nextSignalItems, nextConfig, nextDataStatus, nextMcpStatus, nextAgentChannels, nextAttentionPolicy, nextPairingStatus, nextNodeLinkStatus, nextOutboundDrafts, nextSampleStatus, nextEmailStatus, nextEmailSettings, nextPushStatus, nextPushSettings] = await Promise.all([api.threads(), api.contacts(), api.agentMessages(), api.calls(), api.signalSubscriptions(), api.signalItems(), api.config(), api.dataStatus(), shell.mcpStatus(), shell.agentChannels(), shell.attentionPolicy(), shell.pairingStatus().catch(() => DEFAULT_ANDROID_PAIRING_STATUS), shell.nodeLinkStatus().catch(() => DEFAULT_NODE_LINK_STATUS), api.outboundDrafts().catch(() => [] as OutboundDraft[]), api.sampleStatus().catch(() => undefined), api.emailStatus().catch(() => undefined), shell.emailSettings().catch(() => undefined), api.pushStatus().catch(() => undefined), shell.pushSettings().catch(() => undefined)]);
    setThreads(nextThreads); setContacts(nextContacts); setAgentMessages(nextAgentMessages); setCalls(Array.isArray(nextCalls) ? nextCalls : []); setSignalSubscriptions(nextSignalSubscriptions); setSignalItems(nextSignalItems); setConfig(nextConfig); setDataStatus(nextDataStatus); if (nextMcpStatus) setMcpStatus(nextMcpStatus); if (nextAgentChannels) setAgentChannels(nextAgentChannels); if (nextAttentionPolicy) setAttentionPolicy(nextAttentionPolicy); setPairingStatus(nextPairingStatus || DEFAULT_ANDROID_PAIRING_STATUS); setNodeLinkStatus(nextNodeLinkStatus || DEFAULT_NODE_LINK_STATUS); setOutboundDrafts(Array.isArray(nextOutboundDrafts) ? nextOutboundDrafts : []); if (nextSampleStatus) setSampleStatus(nextSampleStatus); if (nextEmailStatus) setEmailStatus(nextEmailStatus); if (nextEmailSettings) setEmailSettings(nextEmailSettings); if (nextPushStatus) setPushStatus(nextPushStatus); if (nextPushSettings) setPushSettings(nextPushSettings);
  ...
  useEffect(() => {
    ...
    shell.onServerStatus(next => { setStatus(next); setHost(next.baseUrl); });
    return () => { active = false; };
  }, []);
  ...
  useEffect(() => {
    ...
    const update = () => setPresence(current => ({
      ...
      paired_mobile: attentionPolicy?.presence_paired_mobile === "nearby" || attentionPolicy?.presence_paired_mobile === "away" ? attentionPolicy.presence_paired_mobile : "unknown",
      updated_at: new Date().toISOString()
    }));
    const markInput = () => { lastInput = Date.now(); update(); };
  ...
  return <div className="app-shell">
    ...
    {view === "decisions" ? <DecisionInbox messages={agentMessages} attentionPolicy={attentionPolicy} onRead={async id => { try { const result = await api.markAgentMessageRead(id); setAgentMessages(current => current.map(message => message.id === id ? result.message : message)); } catch (cause) { setError(String(cause)); } }} onDismiss={async id => { try { const result = await api.dismissAgentMessage(id); setAgentMessages(current => current.map(message => message.id === id ? result.message : message)); } catch (cause) { setError(String(cause)); } }} onAction={async (id, actionId) => { try { const result = await api.actOnAgentMessage(id, actionId); setAgentMessages(current => current.map(message => message.id === id ? result.message : message)); void notify({ kind: "system", title: "Agent response recorded", body: actionId }); } catch (cause) { setError(String(cause)); } }}/>
      ...
      : view === "mobile" ? <MobileDecisionTerminal messages={agentMessages} attentionPolicy={attentionPolicy} presence={presence} api={api}/>
      : view === "people" ? <People contacts={visibleContacts} search={contactSearch} onSearch={setContactSearch} onAdd={() => setModal({ kind: "contact" })} onEdit={contact => setModal({ kind: "contact-edit", contact })} onMessage={contact => { const thread = threads.find(item => item.canonical_number === contact.number); if (thread) chooseThread(thread.id); else setModal({ kind: "message", number: contact.number }); }}/>
      : view === "outbox" ? <ReviewedOutbox api={api} drafts={outboundDrafts} onEdit={async (id, body) => { try { const result = await api.editOutboundDraft(id, body); setOutboundDrafts(current => current.map(draft => draft.id === id ? result.draft : draft)); } catch (cause) { setError(String(cause)); } }} onApproveSend={async id => { try { const result = await api.approveSendOutboundDraft(id); setOutboundDrafts(current => current.map(draft => draft.id === id ? result.draft : draft)); if (result.error) setError(result.error); else void notify({ kind: "system", title: "External message sent", body: result.draft.to_number }); } catch (cause) { setError(String(cause)); } }} onDeny={async id => { try { const result = await api.denyOutboundDraft(id); setOutboundDrafts(current => current.map(draft => draft.id === id ? result.draft : draft)); } catch (cause) { setError(String(cause)); } }} onSchedule={async (id, at) => { try { const result = await api.scheduleOutboundDraft(id, at); setOutboundDrafts(current => current.map(draft => draft.id === id ? result.draft : draft)); void notify({ kind: "system", title: "Send scheduled", body: result.draft.to_number }); } catch (cause) { setError(String(cause)); } }} onCancelSchedule={async id => { try { const result = await api.cancelOutboundDraftSchedule(id); setOutboundDrafts(current => current.map(draft => draft.id === id ? result.draft : draft)); } catch (cause) { setError(String(cause)); } }} onRefresh={async () => { try { await api.dispatchDueDrafts(); setOutboundDrafts(await api.outboundDrafts()); } catch (cause) { setError(String(cause)); } }}/>
      : view === "channels" ? <Channels threads={threads} calls={calls} signalItems={signalItems} signalSubscriptions={signalSubscriptions} config={config} agentChannels={agentChannels} onView={setView}/>
      : <Settings mobileLocalMode={mobileLocalMode} pairingStatus={pairingStatus} nodeLinkStatus={nodeLinkStatus} config={config} status={status} dataStatus={dataStatus} mcpStatus={mcpStatus} agentChannels={agentChannels} attentionPolicy={attentionPolicy} presence={presence} host={host} onConfigure={() => setModal({ kind: "settings" })} onConsole={() => shell.openExternal("https://console.twilio.com/")} onImport={async () => { try { const next = await shell.importEnvironment(); setStatus(next); setHost(next.baseUrl); setConfig(await new PhoneApi(() => ({ baseUrl: next.baseUrl, apiToken })).config()); void notify({ kind: "system", title: "Environment credentials imported", body: `Using ${next.validation?.phone_number || "the selected Twilio number"}.` }); } catch (cause) { setError(String(cause)); } }} onRemove={async () => { try { const next = await shell.removeCredentials(); setStatus(next); setConfig({ account_sid: false, auth_token: false, phone_number: false, public_base_url: false }); setModal({ kind: "settings" }); } catch (cause) { setError(String(cause)); } }} onToggle={async () => { try { const next = status?.running === false ? (status?.configured ? await shell.startServer({}) : await shell.startLocalOnly({})) : await shell.stopServer(); setStatus(next); } catch (cause) { setError(String(cause)); } }} onBackup={async () => { try { const result = await api.backupData(); setDataStatus(await api.dataStatus()); void notify({ kind: "system", title: "Backup created", body: result.name }); } catch (cause) { setError(String(cause)); } }} onExport={async () => { try { const result = await api.exportData(); void notify({ kind: "system", title: "Sensitive export created", body: result.name }); } catch (cause) { setError(String(cause)); } }} onRestore={async () => { if (!window.confirm("Restore the latest backup? Current data will be preserved in a rollback copy.")) return; try { const result = await api.restoreLatestBackup(); await loadAll(); void notify({ kind: "system", title: "Backup restored", body: result.name }); } catch (cause) { setError(String(cause)); } }} onRetention={async days => { if (!window.confirm(`Delete communication history older than ${days} days? A backup will be created first.`)) return; try { const result = await api.applyRetention(days); await loadAll(); void notify({ kind: "system", title: "Retention complete", body: `${result.deletedMessages} messages, ${result.deletedCalls} calls, and ${result.deletedUploads} uploads removed.` }); } catch (cause) { setError(String(cause)); } }} onAttentionSave={async policy => { try { const saved = await shell.saveAttentionPolicy(policy); setAttentionPolicy(saved); } catch (cause) { setError(String(cause)); } }} onMcpCreate={async () => { try { setMcpStatus(await shell.createMcpToken()); void notify({ kind: "system", title: "MCP token ready", body: "Agent apps can use the ForgeLink token file." }); } catch (cause) { setError(String(cause)); } }} onMcpRevoke={async () => { if (!window.confirm("Revoke the MCP token file? External agent apps will stop reaching ForgeLink.")) return; try { setMcpStatus(await shell.revokeMcpToken()); void notify({ kind: "system", title: "MCP token revoked", body: "External agent apps must be reconfigured before use." }); } catch (cause) { setError(String(cause)); } }} onMcpTest={async () => { try { setMcpStatus(await shell.testMcpBridge()); await loadAll(); void notify({ kind: "system", title: "MCP bridge tested", body: "A local test message was created." }); } catch (cause) { setError(String(cause)); } }} onChannelCreate={async () => { try { const channel = await shell.createAgentChannel({ channel_id: "forgewire", label: "ForgeWire Fabric" }); setAgentChannels(current => [channel, ...current.filter(item => item.channel_id !== channel.channel_id)]); void notify({ kind: "system", title: "Agent channel credential ready", body: "ForgeWire can use the channel token file." }); } catch (cause) { setError(String(cause)); } }} onChannelRotate={async id => { try { const channel = await shell.rotateAgentChannel(id); setAgentChannels(current => current.map(item => item.channel_id === id ? channel : item)); void notify({ kind: "system", title: "Agent channel rotated", body: id }); } catch (cause) { setError(String(cause)); } }} onChannelRevoke={async id => { if (!window.confirm(`Revoke channel ${id}? Agent apps using this channel token will stop creating messages.`)) return; try { const channel = await shell.revokeAgentChannel(id); setAgentChannels(current => current.map(item => item.channel_id === id ? channel : item)); void notify({ kind: "system", title: "Agent channel revoked", body: id }); } catch (cause) { setError(String(cause)); } }} onChannelEnabled={async (id, enabled) => { try { const channel = await shell.setAgentChannelEnabled(id, enabled); setAgentChannels(current => current.map(item => item.channel_id === id ? channel : item)); } catch (cause) { setError(String(cause)); } }} emailStatus={emailStatus} emailSettings={emailSettings} onEmailSave={async values => { try { const saved = await shell.saveEmailSettings(values); setEmailSettings(saved); await loadAll(); void notify({ kind: "system", title: "Email credentials saved", body: "Stored securely; the local service restarted to apply them." }); } catch (cause) { setError(String(cause)); } }} onEmailRemove={async () => { if (!window.confirm("Remove stored email credentials?")) return; try { const saved = await shell.removeEmailSettings(); setEmailSettings(saved); await loadAll(); void notify({ kind: "system", title: "Email credentials removed", body: "The email channel is disabled until reconfigured." }); } catch (cause) { setError(String(cause)); } }} pushStatus={pushStatus} pushSettings={pushSettings} onPushSave={async values => { try { const saved = await shell.savePushSettings(values); setPushSettings(saved); await loadAll(); void notify({ kind: "system", title: "Push credentials saved", body: "Stored securely; the local service restarted to apply them." }); } catch (cause) { setError(String(cause)); } }} onPushRemove={async () => { if (!window.confirm("Remove stored push credentials? This revokes the topic/token and disables push.")) return; try { const saved = await shell.removePushSettings(); setPushSettings(saved); await loadAll(); void notify({ kind: "system", title: "Push credentials removed", body: "The push channel is disabled until reconfigured." }); } catch (cause) { setError(String(cause)); } }} onPushTest={async () => { try { const result = await api.sendTestPush(); void notify({ kind: "system", title: "Test notification sent", body: `Status: ${result.status}.` }); } catch (cause) { setError(String(cause)); } }} sampleStatus={sampleStatus} onSampleLoad={async () => { try { const result = await api.loadSample(); await loadAll(); setSampleStatus({ loaded: result.loaded, counts: result.counts }); void notify({ kind: "system", title: "Sample workspace loaded", body: "Synthetic contacts, agents, and approvals are ready to explore." }); } catch (cause) { setError(String(cause)); } }} onSampleClear={async () => { if (!window.confirm("Remove the sample workspace? Only synthetic sample data is deleted.")) return; try { const result = await api.clearSample(); await loadAll(); setSampleStatus({ loaded: result.loaded, counts: result.counts }); void notify({ kind: "system", title: "Sample workspace cleared", body: "Synthetic sample data was removed." }); } catch (cause) { setError(String(cause)); } }}/>}
    {sampleStatus?.loaded && <div className="toast sample" role="status"><Icon name="alert" size={16}/><span>Sample workspace active — every contact, agent, and approval shown is synthetic.</span></div>}
    ...
    {modal?.kind === "settings" && <ConnectionModal status={status} firstRun={status?.needs_onboarding === true} onClose={closeModal} onValidate={values => shell.validateSettings(values)} onSave={async values => { const next = await shell.startServer(values); setStatus(next); setHost(next.baseUrl); setConfig(await new PhoneApi(() => ({ baseUrl: next.baseUrl, apiToken })).config()); void notify({ kind: "system", title: "Connection settings saved", body: `Validated ${next.validation?.phone_number || "the selected Twilio number"} and restarted the local service.` }); }} onStartLocalOnly={async values => { const next = await shell.startLocalOnly(values); setStatus(next); setHost(next.baseUrl); setConfig(await new PhoneApi(() => ({ baseUrl: next.baseUrl, apiToken })).config()); void notify({ kind: "system", title: "Local-only mode ready", body: "ForgeLink is ready for agent messages and local workflows." }); }}/>}
...
function Rail({ view, running, agentUnreadCount, onView }: { view: View; running: boolean; agentUnreadCount: number; onView(view: View): void }) {
  ...
  const iconFor = (item: View) => item === "decisions" ? "alert" : item === "people" ? "users" : item === "agents" ? "nodes" : item === "channels" ? "inbox" : "settings";
  const labelFor = (item: View) => item === "decisions" ? "Decisions" : item === "people" ? "People" : item === "agents" ? "Agents" : item === "channels" ? "Channels" : "Settings";
  const activeFor = (item: View) => view === item || (item === "channels" && ["messages", "calls", "signals", "mobile", "outbox"].includes(view));
  return <nav className="rail" aria-label="Primary navigation"><div className="brand-mark" title="ForgeLink">F</div><div className="rail-nav">{items.map(item => <button key={item} className={`nav-button ${activeFor(item) ? "active" : ""}`} aria-label={labelFor(item)} onClick={() => onView(item)}><span className="nav-icon-wrap"><Icon name={iconFor(item)}/>{item === "decisions" && agentUnreadCount > 0 && <span className="nav-badge" aria-label={`${agentUnreadCount} unread decisions`}>{Math.min(agentUnreadCount, 9)}{agentUnreadCount > 9 ? "+" : ""}</span>}</span><span>{labelFor(item)}</span></button>)}</div><div className="rail-footer"><span className={`status-dot ${running ? "online" : ""}`} title={running ? "Local service connected" : "Local service stopped"}/></div></nav>;
}
...
function Channels({ threads, calls, signalItems, signalSubscriptions, config, agentChannels, onView }: { threads: Thread[]; calls: CallRow[]; signalItems: SignalItem[]; signalSubscriptions: SignalSubscription[]; config?: ConfigStatus; agentChannels: AgentChannelStatus[]; onView(view: View): void }) {
  ...
  return <main className="content-panel page-panel settings-page"><header className="page-header"><div><span className="eyebrow">Communication surfaces</span><h1>Channels</h1><p>Messages, voice, feeds, and provider configuration live here; decisions stay in their own queue.</p></div></header><div className="settings-grid"><section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="chat"/></div><div><h2>Messages</h2><p>{threads.length} conversations · {unreadThreads} unread.</p></div></div><button className="button primary full" onClick={() => onView("messages")}>Open messages</button></section><section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="phone"/></div><div><h2>Calls</h2><p>{calls.length} call records.</p></div></div><button className="button secondary full" onClick={() => onView("calls")}>Open calls</button></section><section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="rss"/></div><div><h2>Signals</h2><p>{signalSubscriptions.length} sources · {signalItems.length} latest items.</p></div></div><button className="button secondary full" onClick={() => onView("signals")}>Open signals</button></section><section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="phone"/></div><div><h2>Mobile terminal</h2><p>Paired, redacted, signed human decisions for Tauri 2 mobile.</p></div></div><button className="button secondary full" onClick={() => onView("mobile")}>Open mobile terminal</button></section><section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="inbox"/></div><div><h2>Reviewed outbox</h2><p>Review, redact-preview, approve, schedule, or deny agent-drafted external messages.</p></div></div><button className="button secondary full" onClick={() => onView("outbox")}>Open reviewed outbox</button></section><section className="settings-card span-two"><h2>Provider configuration</h2><div className="status-list"><StatusRow label="Local native channel" ready={true}/><StatusRow label="SMS/MMS provider" ready={config?.account_sid && config?.auth_token && config?.phone_number}/><StatusRow label="Voice webhook" ready={voiceReady(config)}/><StatusRow label="Agent channels" ready={agentChannels.some(channel => channel.enabled && channel.configured)}/></div></section></div></main>;
}
...
function ChannelRedactionPreview({ api, title, body }: { api: PhoneApi; title: string; body: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const results = await Promise.all(PREVIEW_CHANNELS.map(async channel => ({ ...channel, preview: await api.previewRedaction(channel.profile, title, body) })));
        if (active) { setRows(results); setError(""); }
      } catch (cause) { if (active) setError(String(cause)); }
    })();
    return () => { active = false; };
  }, [api, title, body]);
  return <div className="redaction-preview" aria-label="Channel redaction preview">{error ? <span className="error-text">{error}</span> : rows.map(row => {
    const notification = row.preview?.notification;
    return <div className="redaction-row" key={row.profile}><span className="redaction-channel">{row.label}</span><span className="redaction-shown">{notification?.title}{notification?.body ? ` — ${notification.body}` : " — [body hidden on this channel]"}</span><span className={`config-pill ${notification?.redacted ? "missing" : "ready"}`}>{notification?.redacted ? "redacted" : "full"}</span></div>;
  })}</div>;
}
...
function OutboundDraftCard({ api, draft, onEdit, onApproveSend, onDeny, onSchedule, onCancelSchedule }: { api: PhoneApi; draft: OutboundDraft; onEdit(id: string, body: string): Promise<void>; onApproveSend(id: string): Promise<void>; onDeny(id: string): Promise<void>; onSchedule(id: string, at: string): Promise<void>; onCancelSchedule(id: string): Promise<void> }) {
  ...
  return <article className={`outbox-card status-${draft.status}`} aria-label={`Draft to ${draft.to_number}`}>
    <div className="outbox-card-head"><div><span className={`badge status-${draft.status}`}>{draft.status}</span> <strong>To {draft.to_number}</strong></div><span className="agent-meta">{draft.agent_id} · {draft.channel_kind}</span></div>
    {editing ? <textarea className="outbox-edit" rows={3} value={body} aria-label="Edit draft body" onChange={event => setBody(event.target.value)}/> : <p className="outbox-body">{draft.body || "(no body)"}</p>}
    <div className="outbox-meta"><span>Firewall: {draft.firewall_decision || "n/a"}</span>{draft.reason && <span>{draft.reason}</span>}{scheduled && draft.scheduled_at && <span>Scheduled for {formatListTime(draft.scheduled_at)}</span>}{draft.last_error && <span className="error-text">{draft.last_error}</span>}</div>
    <button className="button subtle small" aria-expanded={showPreview} onClick={() => setShowPreview(value => !value)}>{showPreview ? "Hide channel preview" : "Preview redaction across channels"}</button>
    {showPreview && <ChannelRedactionPreview api={api} title={`External message to ${draft.to_number}`} body={editing ? body : draft.body}/>}
    {pending && <div className="outbox-actions">{editing
      ? <><button className="button primary small" onClick={async () => { await onEdit(draft.id, body); setEditing(false); }}>Save</button><button className="button subtle small" onClick={() => { setBody(draft.body); setEditing(false); }}>Cancel</button></>
      : <button className="button secondary small" onClick={() => setEditing(true)}>Edit</button>}
      <button className="button primary small" onClick={() => void onApproveSend(draft.id)}>Approve &amp; send</button>
      <button className="button danger small" onClick={() => void onDeny(draft.id)}>Deny</button>
      <label className="outbox-schedule"><span className="sr-only">Schedule send time</span><input type="datetime-local" value={scheduleAt} aria-label="Schedule send time" onChange={event => setScheduleAt(event.target.value)}/><button className="button secondary small" disabled={!scheduleAt} onClick={() => void onSchedule(draft.id, new Date(scheduleAt).toISOString())}>Schedule</button></label>
    </div>}
    {scheduled && <div className="outbox-actions"><button className="button secondary small" onClick={() => void onCancelSchedule(draft.id)}>Cancel schedule</button></div>}
  </article>;
}

function ReviewedOutbox({ api, drafts, onEdit, onApproveSend, onDeny, onSchedule, onCancelSchedule, onRefresh }: { api: PhoneApi; drafts: OutboundDraft[]; onEdit(id: string, body: string): Promise<void>; onApproveSend(id: string): Promise<void>; onDeny(id: string): Promise<void>; onSchedule(id: string, at: string): Promise<void>; onCancelSchedule(id: string): Promise<void>; onRefresh(): Promise<void> }) {
  ...
  return <main className="content-panel page-panel outbox-page"><header className="page-header"><div><span className="eyebrow">External communication</span><h1>Reviewed outbox</h1><p>Agent-drafted external messages wait here for human review. Pending drafts are kept separate from sent messages.</p></div><div className="header-actions"><span className="count-label">{pending} awaiting review</span><button className="button subtle" onClick={() => void onRefresh()}>Refresh</button></div></header>{drafts.length === 0 ? <div className="list-empty"><Icon name="inbox" size={28}/><strong>No drafts</strong><span>Agent-drafted external messages will appear here for review before they are sent.</span></div> : <section className="outbox-lanes">{lanes.map(lane => <section className={`outbox-lane lane-${lane.id}`} key={lane.id} aria-label={lane.label}><div className="section-title"><h2>{lane.label}</h2><span>{lane.items.length}</span></div>{lane.items.length ? <div className="outbox-list">{lane.items.map(draft => <OutboundDraftCard key={draft.id} api={api} draft={draft} onEdit={onEdit} onApproveSend={onApproveSend} onDeny={onDeny} onSchedule={onSchedule} onCancelSchedule={onCancelSchedule}/>)}</div> : <div className="lane-empty">No items</div>}</section>)}</section>}</main>;
}
...
function DeviceHealthBody({ normalized, onClear }: { normalized: NormalizedOperatorStatus; onClear(): void }) {
  ...
  return <><div className="status-list"><StatusRow label={`Status: ${health === "online" ? "Online" : "Degraded"}`} ready={health === "online"}/></div><div className="fatigue-grid"><Metric label="Target" value={status.target || "unknown"}/><Metric label="Android" value={status.device ? `${status.device.android_release} / SDK ${status.device.sdk}` : "unknown"}/><Metric label="Model" value={status.device?.model || "unknown"}/><Metric label="Hardware" value={status.device?.hardware || "unknown"}/><Metric label="Boot completed" value={status.boot?.completed ? "yes" : "no"}/><Metric label="Current user" value={status.activity?.current_user || "unknown"}/><Metric label="Top activity" value={topActivityLabel(status.activity?.top_activity)}/><Metric label="Packages visible" value={status.packages?.count ?? 0}/><Metric label="Network" value={status.network?.summary || "n/a"}/><Metric label="Storage" value={status.storage?.summary || "n/a"}/></div>{status.error && <p className="error-text">{status.error}</p>}<p className="settings-source">Bridge: {status.bridge_version || "unknown"} · Last updated: {status.generated_at || "unknown"} · advisory, read-only.</p><div className="data-actions"><button className="button subtle" onClick={onClear}>Hide status</button></div></>;
}
...
function AndroidDeviceHealth({ api }: { api: PhoneApi }) {
  ...
  const check = async () => {
    setLoading(true);
    try { setStatus(parseOperatorStatus(await api.operatorStatus())); }
    catch (cause) { setStatus(parseOperatorStatus({ ok: false, mode: "operator-status", request_id: "forgelink-op", error: String(cause) })); }
    finally { setLoading(false); }
  };
  return <section className="settings-card device-health" aria-label="Android / Fabric Device Health"><div className="settings-card-head"><div className="settings-icon"><Icon name="nodes"/></div><div><h2>Android / Fabric Device Health</h2><p>Read-only emulator status from the ROM lab bridge. This status is advisory and cannot grant authority or trigger actions.</p></div></div>{status ? <DeviceHealthBody normalized={status} onClear={() => setStatus(null)}/> : <div className="device-health-empty"><p className="settings-source">No status loaded. Check the ROM lab bridge or show a sample.</p></div>}<div className="data-actions"><button className="button secondary" disabled={loading} onClick={() => void check()}>{loading ? "Checking…" : "Check device status"}</button><button className="button subtle" onClick={() => setStatus(parseOperatorStatus(SAMPLE_OPERATOR_STATUS))}>Show sample status</button></div></section>;
}

function MobileDecisionTerminal({ messages, attentionPolicy, presence, api }: { messages: AgentMessage[]; attentionPolicy?: AttentionPolicy; presence: PresenceSnapshot; api: PhoneApi }) {
  ...
  return <main className="content-panel page-panel mobile-page"><header className="page-header"><div><span className="eyebrow">Tauri 2 mobile</span><h1>Mobile Terminal</h1><p>A paired decision terminal for redacted alerts, approval cards, signed responses, presence, emergency contact mode, and device revoke.</p></div><span className={`config-pill ${paired ? "ready" : "missing"}`}>{paired ? <Icon name="check" size={14}/> : <Icon name="alert" size={14}/>} {paired ? "Paired nearby" : "Pairing needed"}</span></header><section className="mobile-terminal-layout"><div className="mobile-preview" aria-label="Mobile decision terminal preview"><div className="mobile-device"><div className="mobile-device-bar"><span>ForgeLink</span><span>{presence.network}</span></div><section className="mobile-alert"><span className="eyebrow">Redacted alert</span><h2>{redactedTitle}</h2><p>{current ? "An agent needs a human decision. Open ForgeLink to review the redacted approval card." : "New approval cards will appear here after pairing."}</p></section><section className="mobile-card"><div className="agent-meta"><span>{current?.source || "desktop"}</span><span>{current?.urgency || "normal"}</span><span>mobile_lock_screen</span></div><h3>{current?.requested_action || current?.title || "Decision card ready"}</h3><p>{current ? "Evidence stays summarized on mobile; full private context remains on desktop." : "Desktop remains the source of truth for private data."}</p><div className="mobile-action-grid"><button className="button primary">Approve</button><button className="button danger">Deny</button><button className="button secondary">Defer</button><button className="button secondary">Request info</button><button className="button secondary span-two">Short reply</button></div></section><section className="mobile-controls"><label className="toggle-row"><input type="checkbox" checked={paired} readOnly/><span>Presence signal</span></label><label className="toggle-row"><input type="checkbox" checked={attentionPolicy?.emergency_contact_bypass === true} readOnly/><span>Emergency contact toggle</span></label><button className="button danger full">Revoke device</button></section></div></div><div className="mobile-flow"><section className="settings-card"><h2>MVP boundaries</h2><div className="status-list"><StatusRow label="Desktop source of truth" ready={true}/><StatusRow label="Redacted mobile alerts" ready={true}/><StatusRow label="Approval card actions" ready={true}/><StatusRow label="No private DB replication" ready={true}/></div></section><section className="settings-card"><h2>Signed return flow</h2><ol className="flow-list">{signedReturnSteps.map(step => <li key={step}>{step}</li>)}</ol></section><section className="settings-card"><h2>Decision actions</h2><div className="mobile-action-list"><span>Approve</span><span>Deny</span><span>Defer</span><span>Request more info</span><span>Short reply</span></div></section><AndroidDeviceHealth api={api}/></div></section></main>;
}
...
function EmailCredentialForm({ status, onSave, onRemove }: { status?: EmailSettingsStatus; onSave(values: EmailSettingsInput): void; onRemove(): void }) {
  ...
  useEffect(() => { if (status) { setHost(status.host); setPort(String(status.port)); setUser(status.user); setFrom(status.from); } }, [status]);
  const save = () => onSave({ host, port: Number(port) || 465, secure: (Number(port) || 465) === 465, user, from: from || user, ...(pass ? { pass } : {}), ...(inbound !== "" ? { inbound_secret: inbound } : {}), ...(action !== "" ? { action_secret: action } : {}) });
  return <div className="form-stack email-cred-form"><Field label="SMTP host"><input value={host} onChange={event => setHost(event.target.value)} placeholder="smtp.provider.example"/></Field><Field label="Port" hint="465 implicit TLS, 587 STARTTLS"><input type="number" value={port} onChange={event => setPort(event.target.value)}/></Field><Field label="Username"><input value={user} onChange={event => setUser(event.target.value)} placeholder="ops@your-domain" autoComplete="off"/></Field><Field label="From address"><input value={from} onChange={event => setFrom(event.target.value)} placeholder="ForgeLink <ops@your-domain>"/></Field><Field label={status?.password_present ? "Password / token (blank keeps current)" : "Password / token"}><input type="password" value={pass} onChange={event => setPass(event.target.value)} autoComplete="new-password"/></Field><Field label={`Inbound webhook secret${status?.inbound_secret_present ? " (set)" : ""}`}><input type="password" value={inbound} onChange={event => setInbound(event.target.value)} autoComplete="new-password"/></Field><Field label={`Quick-action secret${status?.action_secret_present ? " (set)" : ""}`}><input type="password" value={action} onChange={event => setAction(event.target.value)} autoComplete="new-password"/></Field><div className="data-actions"><button type="button" className="button primary" onClick={save}>Save email credentials</button><button type="button" className="button danger" disabled={!status?.configured} onClick={onRemove}>Remove</button></div></div>;
}
...
function PushCredentialForm({ status, onSave, onRemove }: { status?: PushSettingsStatus; onSave(values: PushSettingsInput): void; onRemove(): void }) {
  ...
  useEffect(() => { if (status) { setUrl(status.url || "https://ntfy.sh"); setProfile(status.profile || "lock_screen_safe"); } }, [status]);
  const save = () => onSave({ url, profile, ...(topic ? { topic } : {}), ...(token !== "" ? { token } : {}) });
  return <div className="form-stack push-cred-form"><Field label="Provider URL" hint="ntfy.sh or a self-hosted instance"><input value={url} onChange={event => setUrl(event.target.value)} placeholder="https://ntfy.sh"/></Field><Field label={status?.topic_present ? "Topic (blank keeps current)" : "Topic"} hint="Treated as a secret — use a hard-to-guess value"><input type="password" value={topic} onChange={event => setTopic(event.target.value)} autoComplete="off"/></Field><Field label={`Access token${status?.token_present ? " (set)" : " (optional)"}`}><input type="password" value={token} onChange={event => setToken(event.target.value)} autoComplete="new-password"/></Field><Field label="Redaction profile"><select value={profile} onChange={event => setProfile(event.target.value)}><option value="lock_screen_safe">Lock-screen-safe (recommended)</option><option value="full">Full content</option></select></Field><div className="data-actions"><button type="button" className="button primary" onClick={save}>Save push credentials</button><button type="button" className="button danger" disabled={!status?.configured} onClick={onRemove}>Remove</button></div></div>;
}

function Settings({ mobileLocalMode, pairingStatus, nodeLinkStatus, config, status, dataStatus, mcpStatus, agentChannels, attentionPolicy, presence, host, onConfigure, onConsole, onImport, onRemove, onToggle, onBackup, onExport, onRestore, onRetention, onAttentionSave, onMcpCreate, onMcpRevoke, onMcpTest, onChannelCreate, onChannelRotate, onChannelRevoke, onChannelEnabled, emailStatus, emailSettings, onEmailSave, onEmailRemove, pushStatus, pushSettings, onPushSave, onPushRemove, onPushTest, sampleStatus, onSampleLoad, onSampleClear }: { mobileLocalMode: boolean; pairingStatus: AndroidPairingStatus; nodeLinkStatus: ForgeLinkNodeLinkStatus; config?: ConfigStatus; status?: DesktopStatus; dataStatus?: DataStatus; mcpStatus?: McpStatus; agentChannels: AgentChannelStatus[]; attentionPolicy?: AttentionPolicy; presence: PresenceSnapshot; host: string; onConfigure(): void; onConsole(): void; onImport(): void; onRemove(): void; onToggle(): void; onBackup(): void; onExport(): void; onRestore(): void; onRetention(days: number): void; onAttentionSave(policy: AttentionPolicy): void; onMcpCreate(): void; onMcpRevoke(): void; onMcpTest(): void; onChannelCreate(): void; onChannelRotate(id: string): void; onChannelRevoke(id: string): void; onChannelEnabled(id: string, enabled: boolean): void; emailStatus?: EmailChannelStatus; emailSettings?: EmailSettingsStatus; onEmailSave(values: EmailSettingsInput): void; onEmailRemove(): void; pushStatus?: PushChannelStatus; pushSettings?: PushSettingsStatus; onPushSave(values: PushSettingsInput): void; onPushRemove(): void; onPushTest(): void; sampleStatus?: SampleStatus; onSampleLoad(): void; onSampleClear(): void }) {
  ...
  return <main className="content-panel page-panel settings-page"><header className="page-header"><div><span className="eyebrow">Application</span><h1>Settings</h1><p>Connection health, local data safety, and the environment this app uses.</p></div></header><div className="settings-grid">{mobileLocalMode && <section className="settings-card span-two"><div className="settings-card-head"><div className="settings-icon"><Icon name="phone"/></div><div><h2>Android mobile-local runtime active</h2><p>The Android cockpit is running from app-local runtime state while the desktop local service is unavailable.</p></div></div><div className="status-list"><StatusRow label="Attention policy available" ready={Boolean(attentionPolicy)}/><StatusRow label={`Agent channel metadata: ${agentChannels.length}`} ready={agentChannels.length > 0}/><StatusRow label="No private desktop DB replication" ready={true}/><StatusRow label={`Pairing status: ${pairingStatus.label}`} ready={pairingLabels.ready}/><StatusRow label={`Node link: ${nodeLinkLabels.label}`} ready={nodeLinkLabels.ready}/><StatusRow label={`Lifecycle status: ${lifecycleStatus.redacted_health_label}`} ready={lifecycleReady}/><StatusRow label={`Private data locked: ${lifecycleStatus.private_data_locked ? "yes" : "no"}`} ready={!lifecycleStatus.private_data_locked}/><StatusRow label={`Linked operations paused: ${lifecycleStatus.linked_operations_paused ? "yes" : "no"}`} ready={!lifecycleStatus.linked_operations_paused}/></div><div className="capability-matrix" aria-label="Cross-platform node capability matrix"><h3>Cross-platform node capability matrix</h3><ul><li>{`Platform: ${nodeLinkStatus.platform}`}</li><li>{`Node link: ${nodeLinkLabels.label}`}</li><li>{`Sync mode: ${syncModeLabel}`}</li><li>{`Messages: ${nodeCapabilityLabels.messages}`}</li><li>{`Contacts: ${nodeCapabilityLabels.contacts}`}</li><li>{`Calls: ${nodeCapabilityLabels.calls}`}</li><li>{`Signals: ${nodeCapabilityLabels.signals}`}</li><li>{`Decisions: ${nodeCapabilityLabels.decisions}`}</li><li>{`Comms sync: ${nodeCapabilityLabels.sync}`}</li><li>{`Push notifications: ${nodeCapabilityLabels.push}`}</li><li>{`Device pairing: ${pairingLabels.device}`}</li><li>Private data sync: private-data policy pending</li></ul></div><p className="settings-source">{pairingStatus.detail}</p><p className="settings-source">{nodeLinkStatus.detail}</p><p className="settings-source">{lifecycleStatus.redacted_health_detail}</p><p className="settings-source">Recovery: {lifecycleStatus.recovery_action_hint}</p>{lifecycleStatus.last_seen_at && <p className="settings-source">Last seen: {lifecycleStatus.last_seen_at}</p>}{lifecycleStatus.stale_after && <p className="settings-source">Stale after: {lifecycleStatus.stale_after}</p>}{lifecycleStatus.revoked_at && <p className="settings-source">Revoked at: {lifecycleStatus.revoked_at}</p>}{lifecycleStatus.wipe_request_id && <p className="settings-source">Wipe request: {lifecycleStatus.wipe_request_id}</p>}{lifecycleStatus.wipe_ack_id && <p className="settings-source">Wipe acknowledgement: {lifecycleStatus.wipe_ack_id}</p>}<p className="settings-source">Mobile-local state is limited to safe runtime preferences and redacted metadata; private messages and contacts remain out of this Android-local slice.</p></section>}<section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="settings"/></div><div><h2>Twilio connection</h2><p>Your SMS/MMS provider — credentials stay encrypted in the desktop process.</p></div></div><div className="status-list"><StatusRow label="Account SID" ready={config?.account_sid}/><StatusRow label="Auth token" ready={config?.auth_token}/><StatusRow label="Phone number" ready={config?.phone_number}/><StatusRow label="Public webhook URL" ready={config?.public_base_url}/></div><p className="settings-source">Credential source: <strong>{status?.credential_source || "none"}</strong></p>{status?.configured === false && <p className="settings-source">Local-only mode — agent messages and the local inbox work without a telecom provider; add one above to send/receive SMS.</p>}{status?.environment_import_available && status.credential_source === "environment" && <button className="button secondary full" onClick={onImport}>Import environment credentials securely</button>}<button className="button primary full" onClick={onConfigure}>{status?.configured ? "Update connection" : "Configure connection"}</button>{status?.credential_source === "stored" && <button className="button danger full" onClick={onRemove}>Remove stored credentials</button>}</section><section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="external"/></div><div><h2>Twilio Console</h2><p>Manage numbers, messaging webhooks, and usage.</p></div></div><button className="button secondary full" onClick={onConsole}>Open Twilio Console <Icon name="external" size={16}/></button></section><section className="settings-card"><div className="settings-card-head"><div className="settings-icon"><Icon name="nodes"/></div><div><h2>Channels and providers</h2><p>ForgeLink delivers through provider-neutral channels.</p></div></div><div className="status-list"><StatusRow label="Local (native)" ready={true}/><StatusRow label="Twilio SMS/MMS" ready={status?.configured}/></div><p className="settings-source">Telnyx SMS/MMS is supported via environment configuration; Plivo and Bandwidth are planned. The local channel works with no provider.</p></section><section className="settings-card span-two"><h2>Agent apps / MCP</h2><p>External tools use a separate local token file and can only reach agent-channel routes.</p><div className="status-list"><StatusRow label="Bridge built" ready={mcpStatus?.bridge_built}/><StatusRow label="Token configured" ready={mcpStatus?.configured}/><StatusRow label="Token file present" ready={mcpStatus?.token_file_present}/><StatusRow label="Local API reachable" ready={status?.running !== false}/></div><div className="mcp-paths"><div><span>Token file</span><code>{mcpStatus?.token_file || "..."}</code></div><div><span>MCP server</span><code>{mcpStatus?.bridge_server || "..."}</code></div></div><p className="settings-source">Last rotated: <strong>{mcpStatus?.rotated_at ? formatListTime(mcpStatus.rotated_at) : "never"}</strong>{mcpStatus?.last_test_status ? ` · Last test ${mcpStatus.last_test_status}` : ""}</p><div className="data-actions"><button className="button primary" onClick={onMcpCreate}>{mcpStatus?.configured ? "Rotate token file" : "Create token file"}</button><button className="button secondary" disabled={!mcpStatus?.configured} onClick={onMcpTest}>Test MCP bridge</button><button className="button danger" disabled={!mcpStatus?.configured && !mcpStatus?.token_file_present} onClick={onMcpRevoke}>Revoke token</button></div><div className="install-command-grid">{(["vscode", "claude", "codex", "forgewire"] as const).map(target => <div className="install-command" key={target}><span>{target}</span><code>{commands[target] || "Build MCP status unavailable"}</code></div>)}</div></section><section className="settings-card span-two"><h2>Agent channel credentials</h2><p>Each channel has its own local credential and rate-limit counters.</p><div className="data-actions"><button className="button primary" onClick={onChannelCreate}>Create ForgeWire channel</button></div><div className="channel-list">{agentChannels.length ? agentChannels.map(channel => <div className="channel-row" key={channel.channel_id}><div><strong>{channel.label}</strong><span>{channel.channel_id} · {channel.enabled ? "enabled" : "disabled"} · {channel.configured ? "credential configured" : "credential missing"}</span><code>{channel.token_file || "..."}</code><small>Rejected {channel.rejection_count} · Rate limited {channel.rate_limited_count}</small></div><div className="channel-actions"><button className="button secondary" onClick={() => onChannelRotate(channel.channel_id)}>Rotate</button><button className="button secondary" onClick={() => onChannelEnabled(channel.channel_id, !channel.enabled)}>{channel.enabled ? "Disable" : "Enable"}</button><button className="button danger" onClick={() => onChannelRevoke(channel.channel_id)}>Revoke</button></div></div>) : <div className="empty-inline">No agent channel credentials yet.</div>}</div></section><section className="settings-card span-two"><h2>Attention policy</h2><p>Mode, local presence, and emergency boundaries shape routing, redaction, batching, and escalation.</p><div className="presence-strip" aria-label="Local presence snapshot"><span>Focus: {presence.app_focus}</span><span>Input: {presence.input}</span><span>Network: {presence.network}</span><span>Mobile: {presence.paired_mobile}</span><span>DND: {presence.do_not_disturb ? "on" : "off"}</span></div><div className="attention-grid"><label className="field"><span>Operator mode</span><select value={policyDraft.operator_mode} onChange={event => updatePolicy({ operator_mode: event.target.value as AttentionPolicy["operator_mode"] })}><option value="available">Available</option><option value="focus">Focus</option><option value="driving">Driving</option><option value="sleeping">Sleeping</option><option value="family">Family</option><option value="work">Work</option><option value="emergency_only">Emergency-only</option><option value="offline">Offline</option></select></label><label className="toggle-row"><input type="checkbox" checked={policyDraft.enabled} onChange={event => updatePolicy({ enabled: event.target.checked })}/><span>Enable desktop notifications</span></label><label className="toggle-row"><input type="checkbox" checked={policyDraft.redact_notification_bodies} onChange={event => updatePolicy({ redact_notification_bodies: event.target.checked })}/><span>Redact notification text</span></label><label className="toggle-row"><input type="checkbox" checked={policyDraft.quiet_hours_enabled} onChange={event => updatePolicy({ quiet_hours_enabled: event.target.checked })}/><span>Quiet hours</span></label><label className="toggle-row"><input type="checkbox" checked={policyDraft.quiet_hours_allow_urgent} onChange={event => updatePolicy({ quiet_hours_allow_urgent: event.target.checked })}/><span>Allow urgent agent requests during quiet hours</span></label><label className="toggle-row"><input type="checkbox" checked={policyDraft.presence_enabled} onChange={event => updatePolicy({ presence_enabled: event.target.checked })}/><span>Use local presence signals</span></label><label className="toggle-row"><input type="checkbox" checked={policyDraft.presence_do_not_disturb} onChange={event => updatePolicy({ presence_do_not_disturb: event.target.checked })}/><span>Manual do-not-disturb signal</span></label><label className="toggle-row"><input type="checkbox" checked={policyDraft.emergency_contact_bypass} onChange={event => updatePolicy({ emergency_contact_bypass: event.target.checked })}/><span>Emergency contact bypass</span></label><label className="toggle-row"><input type="checkbox" checked={policyDraft.emergency_agent_requires_policy} onChange={event => updatePolicy({ emergency_agent_requires_policy: event.target.checked })}/><span>Require policy for agent emergency claims</span></label><label className="field"><span>Quiet starts</span><input type="time" value={policyDraft.quiet_hours_start} onChange={event => updatePolicy({ quiet_hours_start: event.target.value })}/></label><label className="field"><span>Quiet ends</span><input type="time" value={policyDraft.quiet_hours_end} onChange={event => updatePolicy({ quiet_hours_end: event.target.value })}/></label><label className="field"><span>SMS</span><select value={policyDraft.sms_notifications} onChange={event => updatePolicy({ sms_notifications: event.target.value as AttentionPolicy["sms_notifications"] })}><option value="all">All SMS</option><option value="off">Off</option></select></label><label className="field"><span>Agent channels</span><select value={policyDraft.agent_notifications} onChange={event => updatePolicy({ agent_notifications: event.target.value as AttentionPolicy["agent_notifications"] })}><option value="high_and_urgent">High and urgent</option><option value="urgent_only">Urgent only</option><option value="all">All agent messages</option><option value="off">Off</option></select></label><label className="field"><span>Trusted signals</span><select value={policyDraft.signal_notifications} onChange={event => updatePolicy({ signal_notifications: event.target.value as AttentionPolicy["signal_notifications"] })}><option value="off">Off</option><option value="all">All signal updates</option></select></label><label className="field"><span>System notices</span><select value={policyDraft.system_notifications} onChange={event => updatePolicy({ system_notifications: event.target.value as AttentionPolicy["system_notifications"] })}><option value="all">All system notices</option><option value="failures_only">Failures only</option><option value="off">Off</option></select></label><label className="field"><span>Paired mobile proximity</span><select value={policyDraft.presence_paired_mobile} onChange={event => updatePolicy({ presence_paired_mobile: event.target.value })}><option value="unknown">Unknown</option><option value="nearby">Nearby</option><option value="away">Away</option></select></label><label className="field span-two"><span>Muted sources or channel IDs</span><textarea rows={3} value={policyDraft.muted_sources.join("\n")} onChange={event => updatePolicy({ muted_sources: parseMutedSources(event.target.value) })}/></label></div><p className="settings-source">Presence signals shown here are local and operator-visible; ForgeLink does not collect hidden location, calendar, or surveillance data.</p><div className="data-actions"><button className="button primary" onClick={() => onAttentionSave({ ...policyDraft, presence_app_focus: presence.app_focus, presence_input: presence.input, presence_network: presence.network })}>Save attention policy</button></div></section><section className="settings-card span-two"><h2>Data safety</h2><p>Schema version {dataStatus?.schema_version ?? "..."}. Backups and exports contain private message and contact data.</p>{dataStatus?.recovered_from && <div className="modal-error" role="alert">A damaged database was quarantined as {dataStatus.recovered_from}. Restore a backup if data is missing.</div>}<div className="data-actions"><button className="button primary" onClick={onBackup}>Create backup</button><button className="button secondary" onClick={onExport}>Export JSON</button><button className="button secondary" disabled={!dataStatus?.latest_backup} onClick={onRestore}>Restore latest backup</button></div><p className="settings-source">Managed backups: <strong>{dataStatus?.backup_count ?? 0}</strong>{dataStatus?.latest_backup ? ` · Latest ${dataStatus.latest_backup}` : ""}</p><div className="retention-row"><label className="field"><span>Keep messages for days</span><input type="number" min={30} max={3650} value={retentionDays} onChange={event => setRetentionDays(Number(event.target.value))}/></label><button className="button danger" onClick={() => onRetention(retentionDays)}>Apply retention</button></div></section><section className="settings-card span-two"><div className="settings-card-head"><div className="settings-icon"><Icon name="chat"/></div><div><h2>Email channel</h2><p>A provider-neutral, durable email channel for non-urgent and long-form communication. Email is a fallback, not the default human-approval loop, and it grants no approval privileges automatically.</p></div></div><div className="status-list"><StatusRow label="Email channel" ready={emailStatus?.configured}/><StatusRow label="SMTP host" ready={emailStatus?.host_present}/><StatusRow label="From address" ready={emailStatus?.from_present}/></div><p className="settings-source">{emailStatus?.configured ? `Email is configured. ${emailStatus.recorded_count} recorded message(s) participate in backup, export, and retention.` : "Email is disabled. Enter SMTP credentials below (stored OS-encrypted) or set FORGELINK_SMTP_* in the environment, then the local service restarts to apply them."}</p><p className="settings-source">Add a person's email under People → edit contact → Contact points (kind: Email) to give them an email identity.</p><EmailCredentialForm status={emailSettings} onSave={onEmailSave} onRemove={onEmailRemove}/></section><section className="settings-card span-two"><div className="settings-card-head"><div className="settings-icon"><Icon name="nodes"/></div><div><h2>Push notifications</h2><p>Lock-screen alerts that tell you something needs you — never the message itself. Push is notification-only and grants no approval authority; you act in the cockpit, not from the notification.</p></div></div><div className="status-list"><StatusRow label="Push channel" ready={pushStatus?.configured}/><StatusRow label="Topic configured" ready={pushStatus?.topic_present}/><StatusRow label={`Access token${pushStatus?.token_present ? "" : " (optional)"}`} ready={pushStatus?.token_present}/></div><p className="settings-source">{pushStatus?.configured ? `Push is configured via ${pushStatus.provider} at ${pushStatus.url}. Redaction profile: ${pushStatus.profile === "full" ? "Full content" : "Lock-screen-safe"}.` : "Push is disabled. Enter a provider URL and topic below (stored OS-encrypted) or set FORGELINK_PUSH_* in the environment, then the local service restarts to apply them."}</p><div className="redaction-preview" aria-label="Push redaction preview"><h3>Redaction preview</h3><p className="settings-source">Lock-screen-safe (default) sends only: <strong>ForgeLink — An approval is waiting in ForgeLink.</strong> No names, message text, or amounts leave the device. Full sends the provided title/body (control-stripped and length-clamped) and must be chosen deliberately.</p></div><div className="data-actions"><button type="button" className="button secondary" disabled={!pushStatus?.configured} onClick={onPushTest}>Send test notification</button></div><PushCredentialForm status={pushSettings} onSave={onPushSave} onRemove={onPushRemove}/></section><section className="settings-card span-two"><div className="settings-card-head"><div className="settings-icon"><Icon name="users"/></div><div><h2>Sample workspace</h2><p>Load an optional synthetic workspace to explore the cockpit without real credentials or data. Every sample record is clearly labeled and uses reserved fictional numbers.</p></div></div><p className="settings-source">Status: <strong>{sampleStatus?.loaded ? "Sample data loaded" : "Not loaded"}</strong>{sampleStatus?.loaded ? ` · ${sampleStatus.counts.contacts} contacts · ${sampleStatus.counts.agents} agents · ${sampleStatus.counts.approvals} approvals` : ""}</p><div className="data-actions"><button className="button primary" disabled={sampleStatus?.loaded} onClick={onSampleLoad}>Load sample workspace</button><button className="button danger" disabled={!sampleStatus?.loaded} onClick={onSampleClear}>Clear sample workspace</button></div></section><section className="settings-card span-two"><h2>Local service</h2><div className="service-address"><code>{host}</code><span>{status?.running === false ? "Stopped" : "Online"}</span></div>{status?.effective_port && status?.configured_port && status.effective_port !== status.configured_port && <p className="settings-source">Port {status.configured_port} was busy; the service is running on <strong>{status.effective_port}</strong>.</p>}{Boolean(status?.backend_restarts) && <p className="settings-source">Auto-recovered restarts: <strong>{status?.backend_restarts}</strong>{typeof status?.last_exit_code === "number" ? ` · last exit code ${status?.last_exit_code}` : ""}</p>}{status?.recovery_message && <div className="modal-error" role="alert">{status.recovery_message}</div>}<button className="button secondary full" onClick={onToggle}>{status?.running === false ? "Start local service" : "Stop local service"}</button></section></div></main>;
}

function ConnectionModal({ status, firstRun, onClose, onValidate, onSave, onStartLocalOnly }: { status?: DesktopStatus; firstRun: boolean; onClose(): void; onValidate(values: Record<string, string | number>): Promise<import("./types").ValidationResult>; onSave(values: Record<string, string | number>): Promise<void>; onStartLocalOnly(values: Record<string, string | number>): Promise<void> }) {
  ...
  async function startLocalOnly() {
    ...
    try { await onStartLocalOnly(values()); onClose(); }
    catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); }
    finally { setBusy(null); }
  }
  return <div className="modal-overlay" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose(); }}><form ref={formRef} className="modal-card" role="dialog" aria-modal="true" aria-label={firstRun ? "Welcome to ForgeLink" : "Twilio connection"} onSubmit={save}><div className="modal-head"><div><div className="eyebrow">{firstRun ? "First-run setup" : "Secure local settings"}</div><h2>{firstRun ? "Start with ForgeLink local" : "Twilio connection"}</h2></div><button className="icon-button" type="button" aria-label="Close" disabled={Boolean(busy)} onClick={onClose}><Icon name="close"/></button></div><div className="modal-body"><p>{firstRun ? "ForgeLink works without a telecom provider. Start local-only for agent approvals and the local human loop, or add Twilio when you want SMS/MMS." : "Test changes against Twilio before saving them locally."}</p>{firstRun && <div className="setup-help"><strong>Provider optional</strong><span>Local-only mode stores no Twilio credentials and keeps the local service on loopback. SMS/MMS can be configured later from Settings.</span><div className="setup-help-links"><ExtLink href="https://console.twilio.com/">Open the Twilio Console</ExtLink></div></div>}<div className="form-stack"><Field label="Account SID" hint={<>On the <ExtLink href="https://console.twilio.com/">Console dashboard</ExtLink> under Account Info — it starts with <code>AC</code>.</>}><input name="account_sid" defaultValue={current?.account_sid} placeholder="ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" required/></Field><Field label="Auth token" hint={<>Revealed next to the Account SID on the <ExtLink href="https://console.twilio.com/">Console dashboard</ExtLink>. Stored with operating-system encryption; leave blank to keep the current token.</>}><input name="auth_token" type="password" required={!current?.auth_token_configured} placeholder={current?.auth_token_configured ? "Configured; enter to replace" : "Enter auth token"}/></Field><Field label="Twilio number" hint={<>An SMS-capable number on your account, in +country format. <ExtLink href="https://console.twilio.com/us1/develop/phone-numbers/manage/incoming">View or get a number</ExtLink></>}><input name="twilio_number" type="tel" defaultValue={current?.twilio_number} placeholder="+15551234567" required/></Field></div><details className="setup-advanced"><summary>Advanced local service</summary><div className="form-stack"><Field label="Public webhook URL" hint="ForgeLink can set this up automatically with a secure tunnel when you connect Twilio. Leave it empty for local-only mode."><input name="public_base_url" type="url" defaultValue={current?.public_base_url} placeholder="Automatic"/></Field><Field label="Local host" hint="The local service stays bound to loopback."><input name="webhook_host" defaultValue={current?.webhook_host || "127.0.0.1"} pattern="127\.0\.0\.1|localhost" required/></Field><Field label="Local port" hint="Default local service port."><input name="webhook_port" type="number" defaultValue={current?.webhook_port || 5055} min={1024} max={65535} required/></Field></div></details>{validated && <div className="validation-success" role="status"><Icon name="check" size={18}/><span><strong>{validated.account_name}</strong><br/>Confirmed {validated.phone_number}</span></div>}{error && <div className="modal-error" role="alert">{error}</div>}</div><div className="modal-actions">{firstRun && <button className="button primary" type="button" disabled={Boolean(busy)} onClick={() => void startLocalOnly()}>{busy === "local" ? "Starting..." : "Start local-only"}</button>}<button className="button secondary" type="button" disabled={Boolean(busy)} onClick={() => void testConnection()}>{busy === "test" ? "Testing..." : "Test connection"}</button><button className={firstRun ? "button secondary" : "button primary"} type="submit" disabled={Boolean(busy)}>{busy === "save" ? "Validating and saving..." : "Save and restart"}</button></div></form></div>;
}
