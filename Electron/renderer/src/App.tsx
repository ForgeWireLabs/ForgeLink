...
import type { AgentAction, AgentChannelStatus, AgentMessage, AttentionEvent, AttentionPolicy, BackendConnection, CallRow, ConfigStatus, Contact, ContactPoint, ContactPolicy, ContactTimelineItem, DataStatus, DesktopStatus, McpStatus, Message, OutboundDraft, PresenceSnapshot, RedactionPreview, SampleStatus, SignalItem, SignalSubscription, Thread, View } from "./types";
...
export function App() {
  ...
  const loadAll = useCallback(async () => {
    const [nextThreads, nextContacts, nextAgentMessages, nextCalls, nextSignalSubscriptions, nextSignalItems, nextConfig, nextDataStatus, nextMcpStatus, nextAgentChannels, nextAttentionPolicy, nextOutboundDrafts, nextSampleStatus] = await Promise.all([api.threads(), api.contacts(), api.agentMessages(), api.calls(), api.signalSubscriptions(), api.signalItems(), api.config(), api.dataStatus(), shell.mcpStatus(), shell.agentChannels(), shell.attentionPolicy(), api.outboundDrafts().catch(() => [] as OutboundDraft[]), api.sampleStatus().catch(() => undefined)]);
