...
export interface ForgeLinkShellBridge {
  ...
  startService(): Promise<DesktopStatus>;
  ...
  selectSmsProvider(provider: "none" | "twilio" | "telnyx"): Promise<SmsProviderSettingsStatus>;
...
function createTauriBridge(invoke: TauriInvoke, listen?: TauriListen): ForgeLinkShellBridge {
  return {
    ...
    startService: () => invoke<DesktopStatus>("forgelink_start_service"),
    ...
    selectSmsProvider: (provider: "none" | "twilio" | "telnyx") => invoke<SmsProviderSettingsStatus>("forgelink_select_sms_provider", { provider }),
    ...
    localIntegrations: () => invoke<LocalIntegrationStatus[]>("forgelink_local_integrations"),
    createLocalIntegration: payload => invokeWithPayload<LocalIntegrationStatus>(invoke, "forgelink_create_local_integration", payload),
    updateLocalIntegration: (integrationId, payload) => invoke<LocalIntegrationStatus>("forgelink_update_local_integration", { integrationId, payload }),
    rotateLocalIntegration: integrationId => invoke<LocalIntegrationStatus>("forgelink_rotate_local_integration", { integrationId }),
    revokeLocalIntegration: integrationId => invoke<LocalIntegrationStatus>("forgelink_revoke_local_integration", { integrationId }),
    setLocalIntegrationEnabled: (integrationId, enabled) => invoke<LocalIntegrationStatus>("forgelink_set_local_integration_enabled", { integrationId, enabled }),
    testLocalIntegration: integrationId => invoke<LocalIntegrationStatus>("forgelink_test_local_integration", { integrationId }),
...
export function getShellBridge(): ForgeLinkShellBridge {
  ...
  if (!bridge) {
    return {
      ...
      startService: unavailable("startService"),
...
export const shell: ForgeLinkShellBridge = {
  ...
  startService: () => getShellBridge().startService(),
