...
| v34 | 041 FAX-007 (Phase 4) | `telnyx_fax_webhook_events.connection_id`/`from_number`/`to_number`/`partial_content` (additive; the bounded provider metadata inbound reception needs, minimized by Phase 3 and now extended); `faxes.document_acquisition_state` (additive; deliberately separate from `faxes.state`/FaxState -- transport lifecycle vs. local-document-safety authority); `fax_inbound_acquisitions` (new table; the durable, restart-safe, provider-scoped acquisition/job authority for inbound document fetch, keyed by `(provider, provider_fax_id)` so at most one active acquisition exists per inbound provider fax) | unreleased |
