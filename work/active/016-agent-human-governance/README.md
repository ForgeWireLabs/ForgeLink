...
- [x] **AGH-028 Pipe Fabric HITL approvals through ForgeLink.** When ForgeLink is installed and reachable, ForgeWire Fabric's human-in-the-loop approval pane automatically routes approvals to ForgeLink as the governed decision surface (evidence packs, audit, redaction, mobile companion), with an explicit operator opt-out and graceful fallback to Fabric's built-in pane when ForgeLink is absent. See decision 0004. **Both sides shipped:** the ForgeLink side (capability discovery + the submit/await/outcome contract) is here; the Fabric side (auto-route held approvals to ForgeLink with `FORGELINK_HITL` opt-out and silent fallback) shipped in `forgewire-fabric` (`fabric-hub`, monorepo commit `ce6e22a3`). Remaining operational follow-up: auto-resolving the held Fabric task from the ForgeLink decision (decision write-back).
...
| 2026-06-24 | AGH-028 Fabric side | Fabric HITL through ForgeLink — Fabric side: `fabric-hub` `forgelink` module auto-routes a held HITL approval to ForgeLink's `agent-governance-v1` contract (evidence-bearing approval request on the agent channel) when configured/reachable; `FORGELINK_BASE_URL`/`FORGELINK_CHANNEL_ID`/`FORGELINK_CHANNEL_TOKEN` config with `FORGELINK_HITL` opt-out; best-effort 3s routing with **silent fallback** to Fabric's built-in pane on any failure (ForgeLink is never a hard dependency); audit records `forgelink_routed`/`forgelink_unavailable`; 5 fabric-hub unit tests + cargo check green; landed in the forgewire monorepo (commit `ce6e22a3`, mirrored to forgewire-fabric) | AGH-028 satisfied (evidence 20260624-agh028-fabric-hitl-routing). **Work item 016 is complete: 28/28 criteria satisfied.** Remaining operational follow-up: auto-resolving the held Fabric task from the ForgeLink decision (decision write-back). |
