...
### Phase 7: Boundary hardening, key management, and agent contract (added 2026-06-18 gap review; AGH-023..027 complete 2026-06-24, AGH-028 deferred to forgewire-fabric)
...
- [ ] **AGH-028 Pipe Fabric HITL approvals through ForgeLink.** When ForgeLink is installed and reachable, ForgeWire Fabric's human-in-the-loop approval pane automatically routes approvals to ForgeLink as the governed decision surface (evidence packs, audit, redaction, mobile companion), with an explicit operator opt-out and graceful fallback to Fabric's built-in pane when ForgeLink is absent. See decision 0004. **Deferred:** the ForgeLink side (capability discovery + the submit/await/outcome contract) ships here; the auto-detection-and-routing of Fabric's HITL pane lives in the `forgewire-fabric` repo and is the remaining cross-repo work.
...
| 2026-06-24 | AGH-028 ForgeLink side | Fabric HITL through ForgeLink — ForgeLink side: MCP-safe `GET /api/governance/capabilities` advertises `{ forgelink, governance, contract: agent-governance-v1, hitl_surface, features, endpoints }` so Fabric/agents auto-detect the governed surface; the submit/await/outcome contract endpoints exist and are documented (decision 0004); HTTP test asserts capability discovery over an MCP token. **Deferred:** the Fabric-side auto-detection and pane routing live in the `forgewire-fabric` repo and are not editable from here | AGH-028 deferred (evidence 20260624-agh028-fabric-hitl-forgelink-side); Phase 7 ForgeLink-side work complete. Work item 016: 27/28 criteria satisfied, AGH-028 deferred to forgewire-fabric. |
