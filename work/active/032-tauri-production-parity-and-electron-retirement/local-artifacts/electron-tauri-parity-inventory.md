...
Verified 2026-09-13 against implementation commit `f9e2a4d`, with the completed
lifecycle slice integrated into `origin/main` at `e7e0a69`. This is the deletion
checklist for TPR-009; `yes` in the final column means the capability still blocks
Electron retirement. Historical WI030 evidence is cited as foundation input, not
as proof of current parity.
...
| Onboarding and local-only startup | `onboarding.js`; `start-local-only` in `main.js` | `local-service.json`, `forgelink_start_local_only`, shared `ConnectionModal` | parity for lifecycle; partial overall | Persist loopback config, require onboarding completion, start the real local backend | Rust fresh-start/status tests; renderer baseline | Packaged profile migration and clean-machine proof remain open | TPR-001/002/003/007 | yes |
| Provider validation | `main.js` → `validateTwilioCredentials`; Telnyx stores/validators | `protected_settings.rs` provider commands and `forgelink_validate_*` | parity for TPR-003 | Validate Twilio/Telnyx through bounded backend/provider APIs and keep fax configuration separate | Full npm suite; renderer/provider tests; Rust protected-settings tests | Telnyx Fax renderer parity remains a later provider slice | TPR-001/003 | yes |
| Credential import/save/remove | `onboarding.js`, `smsProviderSettings.js`, `emailSettings.js`, `pushSettings.js`, `main.js` IPC | `protected_settings.rs` behind Tauri commands and shared shell bridge | parity for TPR-003; intentionally replaced migration | Store protected values in the OS-keyring-wrapped Tauri vault; preserve Electron files and require manual re-entry | Rust vault/corruption/migration tests; renderer canary tests; full npm suite | Automatic Electron safeStorage conversion is intentionally not claimed | TPR-001/003 | yes |
...
| Secure settings surface | Electron main-only IPC and OS-encrypted stores | `protected_settings.rs` and `secure_store.rs` behind `lib.rs` commands | parity for TPR-003 | Keep metadata separate from secret refs; never grant renderer filesystem/process access | Rust metadata/redaction/corruption tests; renderer canary tests; full npm suite | TPR-004/005 native diagnostics and notification work remains open | TPR-001/003 | yes |
| MCP credentials | `main.js` `/api/mcp/*`, `~/.forgelink/api.token`, status/install command | `protected_settings.rs` `/api/mcp/*`, encrypted ref, and 0600 compatibility token | parity/intentionally_replaced | Vault is source of truth; materialize only the bounded external-consumer token file atomically | Backend/renderer MCP tests; Rust protected-settings tests | Compatibility file is plaintext by external-consumer contract and remains desktop-only | TPR-001/003 | yes |
| Agent-channel credentials | `main.js` `/api/agent-channels/*`, token files | `forgelink_agent_channels` backend API plus encrypted refs and 0600 compatibility files | parity/intentionally_replaced | Keep mobile metadata-only; desktop stores secret in the protected vault and materializes only the external token file | Backend/renderer agent-channel tests; Rust mobile boundary tests | Native notification delivery remains TPR-004 | TPR-001/003 | yes |
| Email settings | `emailSettings.js`, main IPC, backend env bridge | `protected_settings.rs` and `forgelink_email_settings` | parity for TPR-003 | Port metadata and presence flags while injecting SMTP secrets only into the child backend environment | Full npm email/security tests; renderer canary test; Rust tests | Tauri native notification/diagnostic UX remains later work | TPR-001/003 | yes |
| Push settings | `pushSettings.js`, main IPC, backend env bridge | `protected_settings.rs` and `forgelink_push_settings` | parity for TPR-003; partial overall | Encrypt topic/token and keep lock-screen-safe metadata; native delivery remains a separate slice | Full npm push/security tests; renderer canary test; Rust tests | Native Tauri delivery evidence remains TPR-004 | TPR-001/003/004 | yes |
| Linked-node identity operations | Electron renderer uses shared API/bridge; Tauri `node_identity*.rs` orchestrates vault/backend | Tauri commands in `lib.rs`, shared encrypted store, and loopback lifecycle | parity for TPR-002/003 | Reuse the shared AES-GCM/keyring primitive while keeping lifecycle metadata and private keys separate | Rust node identity/lifecycle and secure-store tests; WI030 evidence | Packaged clean-machine proof remains open | TPR-001/002/003/007 | yes |
| Diagnostics/support | Shared backend `/api/diagnostics`; renderer `PhoneApi` | Same endpoint becomes available only after manager starts backend; Tauri status is redacted | partial; TPR-003 secret exclusion parity | Preserve backend API and add Tauri support report/native diagnostics wrapper later | Backend diagnostics/export/security tests; Rust status redaction tests | Tauri shell packaging/support report UX unproven | TPR-001/003/005 | yes |
...
| Environment construction | `main.js` passes API token, provider env, data dir/version | `local_service.rs` clears inherited env, restores a safe OS allow-list, and injects only protected provider values plus runtime config | parity for TPR-003 boundary | Keep secrets transient in the child environment; suppress child stdout/stderr and bound errors | Rust spawn implementation; status/error redaction tests; full npm security suite | Tauri support-report plumbing remains TPR-005 | TPR-001/002/003/005 | yes |
| Authentication token | `randomBytes(32).toString("base64url")` in Electron main | Manager creates 32-byte base64url token and injects it only into child/connection command | parity for TPR-002/003 boundary | Keep launch-only bearer contract; never serialize into public status or debug output | Rust serialized-status/debug redaction test; backend `/health` contract | This is a per-run local API token, not a provider secret | TPR-001/002/003 | yes |
...
| OS credential storage | Electron `safeStorage` in onboarding/email/push/provider stores | `secure_store.rs` AES-256-GCM files with `FLPSV001` and OS-keyring wrapping key; linked identity remains `FLNIV001` | parity for TPR-003 | Hash opaque refs into bounded filenames; use random nonce/AAD, atomic writes, private directories, and zeroizing reads | Rust secure-store corruption/path/nonce tests; full security suite | Public signing and packaged filesystem ACL proof remain later | TPR-003 | yes |
...
| Existing configuration | Electron settings/userData + backend startup | Tauri app-data `local-service.json` plus protected metadata/vault; legacy candidates are detected and preserved | partial/intentionally_replaced | Show an explicit manual re-entry notice; do not attempt to decrypt Electron safeStorage, and never overwrite legacy files | Rust migration-preservation test; renderer migration-notice test | Operator must re-enter protected credentials before Tauri becomes configured | TPR-001/003/007 | yes |
...
| Diagnostics/private evidence | Electron security scanner and backend redacted diagnostics | Tauri protected status omits values; child output is suppressed and errors are bounded | parity for TPR-003 secret exclusion; partial overall | Keep canary checks and add a Tauri support-report wrapper under TPR-005 | Rust status/error tests; renderer canaries; full npm security/export suite | Tauri support-report UX and packaged recovery remain TPR-005/007 | TPR-003/005 | yes |
...
remaining TPR-004 through TPR-008 work instead of silently treating scaffold
...
## TPR-003 secure-storage conclusion

TPR-003 is satisfied for the protected-settings boundary. Tauri persists redacted
metadata in `protected-settings.json` and stores secret material in the separate
`FLPSV001` AES-256-GCM vault under `protected-vault/`. The vault key is held by the
OS keyring under the ForgeLink protected-settings service/account; each record uses
an opaque namespaced reference, hashed filename, random nonce, reference AAD,
atomic replacement, private-directory permissions, and zeroizing reads. The
existing linked-node identity store uses the same primitive with its compatible
`FLNIV001` format.

The boundary covers Twilio, Telnyx SMS, email, push, MCP, agent-channel,
local-integration, and linked-node lifecycle credentials. Tauri clears inherited
environment variables before starting the backend, restores only a safe OS/runtime
allow-list, injects required provider values transiently, suppresses child output,
and returns only presence flags or redacted metadata to the renderer. Renderer
secret forms are write-only and are canary-tested after save. Mobile cfg paths do
not create this desktop store or receive its secrets.

MCP, agent-channel, and local-integration consumers that require a file receive a
0600 atomic compatibility token under the desktop data directory. Those files are
plaintext by the external-consumer contract, are never the source of truth, are not
returned as secret contents to the renderer, and are never materialized on mobile.

Electron `safeStorage` blobs are not automatically decrypted by this Tauri store.
Legacy settings candidates are detected and preserved, and the renderer directs
the operator through manual re-entry. This is an intentional migration strategy
with no silent credential loss; automatic conversion remains outside TPR-003.
TPR-004 through TPR-011 remain open, and Electron remains a supported compatibility
shell.
