...
- [x] **AGH-014 Add Decision Memory.** Convert repeated human decisions into
...
- [x] **AGH-015 Add outcome callbacks.** Require agents to report what happened
  after approval.
...
### Phase 6: Communication firewall and consent (complete 2026-06-24)

- [x] **AGH-019 Add communication firewall rules.** Let the operator define how
...
- [x] **AGH-020 Add draft-don't-send mode.** For external channels, support safe
...
- [x] **AGH-021 Add external-contact consent ledger.** Track whether agents may
...
- [x] **AGH-022 Add redaction profiles.** Define what request details are allowed
...
### Phase 7: Boundary hardening, key management, and agent contract (added 2026-06-18 gap review; complete 2026-06-24)

- [x] **AGH-023 Harden public ingress / tunnel boundary.** Reconcile the automatic public webhook tunnel (item 014) with private-first: document the inbound attack surface, ensure only signature-validated provider requests reach handlers, rate-limit and lock down exposed webhook routes, scope what the tunnel exposes, and record a decision on the boundary. See decision 0003.
- [x] **AGH-024 Treat agent-supplied content as untrusted.** Evidence packs, approval text, and agent messages are untrusted input: labeled agent-provided/unverified, never auto-executed or auto-trusted, sanitized, with an approval surface that resists spoofed system text, fabricated urgency, and look-alike operator prompts.
- [x] **AGH-025 Define decision/audit key management.** Key and device-key generation, OS-backed storage, rotation, lost-device revocation, and recovery behind decision signing and the tamper-evident chain; state the real integrity guarantee. See decision 0016.
- [x] **AGH-026 Specify the agent-facing governance contract.** How agents submit a request, await a decision, and receive the outcome, mapped onto the MCP bridge and ForgeWire Fabric dispatch/await_result, including timeouts, deferral, and outcome reporting. See decision 0004.
- [x] **AGH-027 Add an end-to-end governance-loop integration test.** request -> risk -> evidence -> decision -> outcome -> audit -> replay, proving the lifecycle holds together beyond unit tests.

- [x] **AGH-028 Pipe Fabric HITL approvals through ForgeLink.** When ForgeLink is installed and reachable, ForgeWire Fabric's human-in-the-loop approval pane automatically routes approvals to ForgeLink as the governed decision surface (evidence packs, audit, redaction, mobile companion), with an explicit operator opt-out and graceful fallback to Fabric's built-in pane when ForgeLink is absent. See decision 0004. **Both sides shipped:** the ForgeLink side (capability discovery + the submit/await/outcome contract) is here; the Fabric side (auto-route held approvals to ForgeLink with `FORGELINK_HITL` opt-out and silent fallback, plus **decision write-back** that resolves the held Fabric approval from the ForgeLink decision) shipped in `forgewire-fabric` (`fabric-hub`, monorepo commits `ce6e22a3` + `5769275c`). The loop is closed end to end.
...
| 2026-06-24 | AGH-019 complete | Communication firewall: schema v21 `communication_firewall_rules` (per decision 0011) holds operator policy (agent/contact/channel-kind scope -> block/draft_only/require_approval/allow); `evaluateCommunicationFirewall` picks the most specific enabled matching rule, breaking ties toward the more restrictive decision, and falls back to the draft-don't-send default when no rule matches; enforced before any external dispatch; operator-only `GET/POST /api/communication-firewall`, `GET /api/communication-firewall/evaluate` (dry-run preview), and `DELETE /api/communication-firewall/:id`; renderer `CommunicationFirewallRule`/`FirewallEvaluation` types; `docs/communication-firewall.md`; DB + HTTP tests cover specificity/restrictiveness, disable/delete, the block path, and operator-only scoping | AGH-019 satisfied (evidence 20260624-agh019-communication-firewall). Paired with draft-don't-send (AGH-020). |
| 2026-06-24 | AGH-020 complete | Draft-don't-send: schema v21 `agent_outbound_drafts` + `outbound_draft_events`; agents submit external messages over their channel credential at `POST /api/agent-channels/:id/outbound-drafts`, which consults the firewall — a `block` rule is refused (403), an `allow` rule sends immediately (explicit, audited direct-send authority), and every other decision (default included) parks a pending draft; operator-only reviewed outbox `GET /api/outbound-drafts`, `GET /api/outbound-drafts/:id/events`, and `POST .../edit|deny|approve-send`; approve-send re-checks the firewall, records the explicit approval, then dispatches through the channel registry and lands the message in the thread; every lifecycle step is audited (created/edited/approved/sent/denied/failed); renderer `OutboundDraft`/`OutboundDraftEvent` types; `docs/communication-firewall.md`; DB + HTTP tests cover the default draft, edit/approve/send, deny terminality, the allow auto-send, the block refusal, and operator/agent scoping (95 backend tests green) | AGH-020 satisfied (evidence 20260624-agh020-draft-dont-send). Phase 6 continues with external-contact consent ledger (AGH-021) and redaction profiles (AGH-022). |
| 2026-06-24 | AGH-021 complete | External-contact consent ledger: schema v22 `consent_ledger` (per decision 0011) records per-contact (and optionally per-agent) consent — allowed topics/channels/hours, requires_review, consent_source, last_reviewed_at — with `UNIQUE(contact_id, agent_id)`; `evaluateContactConsent` returns no direct contact for an unknown contact or a contact with no record, withholds while `requires_review`, and otherwise enforces channel/topic/allowed-hours (UTC, wrap-around aware), with an agent-specific record beating the any-agent wildcard; wired as the second gate in `createOutboundDraft` so an `allow` firewall rule only auto-sends when consent also permits (unknown external contacts default to no direct agent contact); operator-only `GET/POST /api/consent`, `GET /api/consent/evaluate`, `POST /api/consent/:id/review`, `DELETE /api/consent/:id`; renderer `ContactConsent`/`ContactConsentDecision` types; `docs/communication-firewall.md`; DB + HTTP tests cover the unknown/no-record/requires-review/channel/topic/hours paths, the wildcard override, review/delete, the two-gate auto-send, and operator-only scoping | AGH-021 satisfied (evidence 20260624-agh021-consent-ledger). Paired with redaction profiles (AGH-022). |
| 2026-06-24 | AGH-022 complete | Redaction profiles: canonical `REDACTION_PROFILES` (desktop_full, mobile_lock_screen, email_summary, sms_fallback, status_only) with per-surface field rules; `redactionProfile` resolves by id and fails closed to the most restrictive profile for an unknown id; `redactEvidencePack` and `redactNotification` render evidence packs and notifications per the selected profile; the approval replay (AGH-017) was refactored to render evidence/body/comment through the profile spec instead of a binary desktop_full check; operator-only `GET /api/redaction-profiles` and `POST /api/redaction-profiles/preview`; renderer `RedactionProfileSpec` type; `docs/approval-requests.md`; DB + HTTP tests cover each profile's evidence/notification redaction, the fail-closed default, the replay integration (summary-only, status-only, unknown), and the preview endpoint (97 backend tests green) | AGH-022 satisfied (evidence 20260624-agh022-redaction-profiles). Phase 6 (communication firewall and consent) is complete; remaining open work is Phase 7 (boundary hardening, key management, agent contract, AGH-023..028). |
| 2026-06-24 | AGH-023 complete | Public ingress boundary: per-process per-minute rate limit (120/min) on all `/webhooks/*` enforced before signature handling (429 on flood); confirmed the provider-signature gate runs before handler logic and that `/api/*` stays credential-gated (401) over the public ingress; `docs/ingress-boundary.md` documents the residual attack surface per decision 0003; HTTP test floods the webhook ingress and asserts 429 after signature-rejected 403s plus private-route 401 | AGH-023 satisfied (evidence 20260624-agh023-ingress-boundary). |
| 2026-06-24 | AGH-024 complete | Untrusted agent content: `sanitizeAgentText` strips ASCII control chars (incl. NUL) and zero-width/bidi spoofing characters and defangs look-alike system/operator/forgelink line prefixes (`[agent] SYSTEM:`); `AGENT_CONTENT_PROVENANCE` labels agent-supplied content; `GET /api/agent-messages` tags every row `provenance: agent_unverified` and the approval replay sanitizes + labels the request title/intent/body; approvals already never auto-decide and trust is never auto-granted; DB + HTTP tests cover control/zero-width stripping, prefix defang, length bound, provenance labeling, and sanitized replay | AGH-024 satisfied (evidence 20260624-agh024-untrusted-agent-content). |
| 2026-06-24 | AGH-025 complete | Decision/audit key management: decision 0016 states the honest guarantee (local append-only tamper-evidence, not non-repudiation); schema v23 `device_keys` registry (per decision 0011) records device identities + public-key refs with `registerDeviceKey`/`rotateDeviceKey`/`revokeDeviceKey`; private keys stay in OS-backed storage (never in the DB/exports/logs); rotation keeps identity, revocation keeps history; operator-only `GET/POST /api/device-keys` and `POST /api/device-keys/:id/rotate|revoke`; renderer `DeviceKey` type; DB + HTTP tests cover register/rotate/revoke and operator-only scoping | AGH-025 satisfied (evidence 20260624-agh025-device-key-management). |
| 2026-06-24 | AGH-026 complete | Agent-facing governance contract (`agent-governance-v1`): `docs/agent-governance-contract.md` maps submit (channel `messages` + evidence pack) -> await (`GET /api/agent-messages/:id/status`, MCP-safe, redacted: decided/decision/authority_granted/outcome/final_state, no operator identity or evidence) -> outcome (`POST .../outcome`), with timeout/deferral via the request's declared behavior, mapped onto Fabric dispatch/await_result per decision 0004; `agentRequestStatus` DB method; DB + HTTP tests cover the redacted status poll and MCP-token reachability | AGH-026 satisfied (evidence 20260624-agh026-agent-governance-contract). |
| 2026-06-24 | AGH-027 complete | End-to-end governance loop integration test: one test drives request -> persisted risk -> evidence -> decision (authority grant) -> outcome -> audit (all four entry types chained and verifying) -> replay (ordered steps + final state) -> agent status, then tampers a decided record and asserts chain verification fails | AGH-027 satisfied (evidence 20260624-agh027-governance-loop-integration). |
| 2026-06-24 | AGH-028 ForgeLink side | Fabric HITL through ForgeLink — ForgeLink side: MCP-safe `GET /api/governance/capabilities` advertises `{ forgelink, governance, contract: agent-governance-v1, hitl_surface, features, endpoints }` so Fabric/agents auto-detect the governed surface; the submit/await/outcome contract endpoints exist and are documented (decision 0004); HTTP test asserts capability discovery over an MCP token | AGH-028 ForgeLink side complete (evidence 20260624-agh028-fabric-hitl-forgelink-side). |
| 2026-06-24 | AGH-028 Fabric side | Fabric HITL through ForgeLink — Fabric side: `fabric-hub` `forgelink` module auto-routes a held HITL approval to ForgeLink's `agent-governance-v1` contract (evidence-bearing approval request on the agent channel) when configured/reachable; `FORGELINK_BASE_URL`/`FORGELINK_CHANNEL_ID`/`FORGELINK_CHANNEL_TOKEN` config with `FORGELINK_HITL` opt-out; best-effort 3s routing with **silent fallback** to Fabric's built-in pane on any failure (ForgeLink is never a hard dependency); audit records `forgelink_routed`/`forgelink_unavailable`; 6 fabric-hub unit tests + cargo check green; landed in the forgewire monorepo (commits `ce6e22a3` routing + `5769275c` write-back, mirrored to forgewire-fabric). Decision write-back closes the loop: `GET /approvals/{id}` polls ForgeLink's status (`fabric-<approval_id>`) and resolves the held Fabric approval approved/denied (needs `FORGELINK_MCP_TOKEN`), audited as `forgelink_decision_synced` | AGH-028 satisfied (evidence 20260624-agh028-fabric-hitl-routing). **Work item 016 is complete: 28/28 criteria satisfied, loop closed end to end.** |
