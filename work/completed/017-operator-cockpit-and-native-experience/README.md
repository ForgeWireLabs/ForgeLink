...
- [x] **OCX-016 Build the killer demo.** Create a reproducible local demo that
...
- [x] **OCX-017 Add public-facing screenshots and narrative.** Prepare product
...
| 2026-06-29 | OCX-016 complete | Added a reproducible synthetic killer demo (`Electron/scripts/killer-demo.js`, `npm run demo`) that drives the full lifecycle against a real local backend with no telecom credentials: agent release-approval with evidence pack → Decisions surface → redacted mobile alert (mobile_lock_screen) → operator approval (recorded) → agent outcome (action_succeeded) → audited replay (chain verified). Covered by `killer-demo.test.js` (added to the test suite) and `docs/killer-demo.md`. Backend build, demo test, full Electron suite, and RepoPact/local validation passed. | OCX-016 satisfied (evidence 20260629-ocx016-killer-demo). |
| 2026-06-29 | OCX-017 complete | README now leads with the human-boundary cockpit (Decisions/People/Agents/Channels) with telecom providers framed as adapters, and links the demo and `docs/public-narrative.md` (positioning + synthetic screenshot index). The visual-smoke harness now captures the four cockpit surfaces from synthetic, body-redacted data (`npm run screenshot` → `ui-cockpit-*.png`). Narrative and the reproducible synthetic/redacted screenshot pipeline are in-repo; PNG capture is a desktop-session command (no display in this/CI env). Scripts syntax-checked; full Electron suite and RepoPact/local validation passed. | OCX-017 satisfied (evidence 20260629-ocx017-narrative-screenshots). All 017 acceptance criteria are now satisfied. |
